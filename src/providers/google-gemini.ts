...
export interface GoogleContentClient {
  models: {
...
export async function generateGoogleContent(
  ...
  if (request.outputType === "image" || request.outputType === "audio") {
    throw new OrchaError(
      ...
      `Google Gemini does not support "${request.outputType}" as a direct agent output.`,
    );
  }
...
function toProviderResponse(
  ...
  for (const part of response.parts) {
    if (part.functionCall) {
      ...
      if (!call.name) {
        throw new Error("Google Gemini returned a function call without a name.");
      }
    ...
    throw new Error("Google Gemini returned an unsupported response part.");
  }
  ...
  for (const message of messages) {
    ...
    contents.push({
      ...
      parts: message.results.map((result) => {
        ...
        if (!call) {
          throw new Error(
            `Google Gemini could not match tool result "${result.callId}" to a prior function call.`,
          );
        }
        return {
          functionResponse: {
...
function toGoogleUserPart(content: MessageContent): Part {
  ...
  if (!isGoogleFileUri(content.fileUri)) {
    throw new OrchaError(
      ...
      "Google Gemini requires media to use a Gemini Files API URI, a Google Cloud Storage URI, or an inline data URI.",
    );
  }
...
function toGoogleAssistantPart(
  ...
  currentProvider: "googlegenai" | "vertexai",
): Part {
  if (block.type === "text") {
    return { text: block.text };
  }
  if (block.type === "reasoning") {
    if (messageProvider !== currentProvider) {
      return { text: block.text };
    }
    return {
      text: block.text,
  ...
  const replay =
    messageProvider === currentProvider ? block.replay : undefined;
  return {
    functionCall: {
      name: block.name,
...
function parseDataUri(uri: string): {
  ...
  if (!match) {
    throw new OrchaError(
      "unsupported_content_type",
      "Google Gemini inline media must be a base64-encoded data URI.",
    );
  }
  return {
    mimeType: match[1] ?? "",
